...
def build_right_panel():
    """
    Construye el panel derecho (Árbol de contenido del curso - Estilo IPN).
    """
    ui["right_panel"] = ctk.CTkFrame(
        ui["body"], fg_color="white", corner_radius=0, width=280,
    ...
    header = ctk.CTkFrame(ui["right_panel"], fg_color="transparent", height=50)
    header.pack(fill="x")
    ...
    # Línea divisora
    ctk.CTkFrame(ui["right_panel"], height=1, fg_color=COLOR_BORDE,
                 corner_radius=0).pack(fill="x")

    # Contenedor con scroll para el temario
...
def refresh_right_panel(subject: str):
    ...
    if not units:
        ctk.CTkLabel(
            ...
            font=ctk.CTkFont(size=11, slant="italic"), text_color="#94A3B8",
...
# --- Funciones Privadas para construir el árbol jerárquico ---

def _add_unit_row(unit_data: dict):
    expanded = {"v": True}
    children_frame = ctk.CTkFrame(ui["tree_scroll"], fg_color="transparent")

    unit_row = ctk.CTkFrame(ui["tree_scroll"], fg_color="transparent", height=32)
    unit_row.pack(fill="x", padx=4, pady=(6, 0))
    ...
    # Flecha de Unidad en Oro
    arrow = ctk.CTkLabel(
        unit_row, text="▼", width=20,
        font=ctk.CTkFont(size=10), text_color=COLOR_ORO, cursor="hand2",
    )
    arrow.pack(side="left")
    arrow.bind("<Button-1>", lambda e: toggle())

    # Nombre de Unidad en Guinda Negrita
    label = ctk.CTkLabel(
        unit_row,
        text=unit_data["unidad"],
        font=ctk.CTkFont(family="Segoe UI", size=12, weight="bold"),
        text_color=COLOR_GUINDA, anchor="w", cursor="hand2",
    )
...
def _add_tema_row(parent, tema_data: dict):
    ...
    tema_row = ctk.CTkFrame(parent, fg_color="transparent", height=26)
    tema_row.pack(fill="x", padx=4, pady=(1, 0))
    ...
    # Espaciador para jerarquía
    ctk.CTkFrame(tema_row, width=18, fg_color="transparent").pack(side="left")

    # Flecha de Tema en Oro suave
    arrow = ctk.CTkLabel(
        tema_row, text="▼", width=16,
        font=ctk.CTkFont(size=9), text_color=COLOR_ORO, cursor="hand2",
    )
    arrow.pack(side="left")
    arrow.bind("<Button-1>", lambda e: toggle())
    ...
    label = ctk.CTkLabel(
        ...
        text_color="#334155", anchor="w", cursor="hand2"
    )
    label.pack(side="left", fill="x", expand=True)
    label.bind("<Button-1>", lambda e: toggle())
...
def _add_subtema_row(parent, text: str):
    ...
    # Doble espaciador para subtemas
    ctk.CTkFrame(row, width=42, fg_color="transparent").pack(side="left")

    # Viñeta en Oro
    ctk.CTkLabel(
        row, text="•", width=12,
        font=ctk.CTkFont(size=14), text_color=COLOR_ORO,
    ).pack(side="left")
